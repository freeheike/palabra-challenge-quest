...
import { spanishReadings, japaneseReadings, englishReadings } from '@/data/readings';
...
const ReadingPassage: React.FC = () => {
  const { currentPassage, currentLanguage, highlightedSentenceIndex, translationItemCount, useTranslationItem } = useGame();
    ...
    <div className="prose max-w-none">
      <h2 className="text-2xl font-semibold text-spanish-text mb-4">
        <JapaneseText 
          ...
          kana={currentLanguage === 'japanese' ? passage.sentenceKana?.[0] || '' : ''}
      ...
      <div className="text-lg leading-relaxed bg-spanish-background p-6 rounded-lg shadow-md min-h-[200px] max-h-[500px] overflow-y-auto">
        {paragraphs.map((paragraph, paragraphIndex) => {
          ...
          return (
            <div key={paragraphIndex} className="mb-6 last:mb-0 relative w-full">
                ...
                {visibleSentenceIndexes.map((sentenceIndex) => {
                  ...
                  return (
                    <div key={sentenceIndex} className="mb-4 last:mb-0 w-full">
                      <span 
                        className={`flex items-start group w-full ${highlightedSentenceIndex === sentenceIndex ? 'bg-yellow-100 -mx-2 px-2 py-1 rounded-md' : ''}`}
                              ...
                              <div className="flex items-center justify-between">
                                <div className="text-xs text-gray-400 italic">
                                  {showRomaji[sentenceIndex] 
                                    ? passage.sentenceRomaji?.[sentenceIndex.toString()]
                                    : passage.sentenceKana?.[sentenceIndex.toString()] || getKanaForSentence(sentence)}
                                </div>
                                <Button
                                  variant="outline"
                                  size="sm"
                                  className="h-6 text-xs px-2 py-0 border-gray-300 hover:bg-gray-100"
                                  onClick={() => setShowRomaji(prev => ({...prev, [sentenceIndex]: !prev[sentenceIndex]}))}
                                >
                                  {showRomaji[sentenceIndex] ? '显示假名' : '显示罗马音'}
                                </Button>
                              </div>
                            </div>
                          ) : (
                      ...
                      {translatedSentences[sentenceIndex] && passage.sentenceTranslations?.[sentenceIndex] && (
        ...
        <div className="mt-8">
          <h3 className="text-xl font-bold mb-4">
            {currentLanguage === 'japanese' ? (
              <div className="flex flex-col gap-1">
                <div className="flex flex-wrap gap-1 w-full">
                  {getWordsFromSentence(passage.question, 'japanese').map((word, wordIndex) => {
                    if (/^[、。！？]$/.test(word)) {
                      return <span key={`question-${wordIndex}`}>{word}</span>;
                    }
                    return (
                      <ClickableWord 
            ...
            {passage.options.map((option, index) => (
              <div key={index} className="p-2 border rounded">
                {currentLanguage === 'japanese' ? (
                  <div className="flex flex-col gap-1">
                    <div className="flex flex-wrap gap-1 w-full">
                      {getWordsFromSentence(option, 'japanese').map((word, wordIndex) => {
                        if (/^[、。！？]$/.test(word)) {
                          return <span key={`option-${index}-${wordIndex}`}>{word}</span>;
                        }
                        return (
                          <ClickableWord 
                    ...
                    <div className="text-xs text-gray-400 italic">
                      {passage.sentenceRomaji?.[passage.text.split('。').length + 1 + index] || ''}
                    </div>
                  </div>
                ) : (
                  option
